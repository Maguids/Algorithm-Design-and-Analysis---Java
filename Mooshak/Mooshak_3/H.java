...
public class H {
        ...
        while(!q.isEmpty()) {
            ...
            for (Edge adj : g.adjs_no(cur)) {
                ...
                if (!ok[neighbor] && adj.value() > dist[neighbor]) {
                    ...
                    pai[neighbor] = cur;
                    q.increaseKey(neighbor, dist[neighbor]);
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < ligacoes; i++) {
            ...
            int d = stdin.nextInt();
            int r = stdin.nextInt();
            g.insert_new_edge(o,d,r);
            g.insert_new_edge(d,o,r);
        }
        List<int[]> result = prim(g, 1);
        int[] pai = result.get(0);
        int[] dist = result.get(1);
        int control = 1;    // todos tem pai
        int max = 0;
        for(int i = 2; i <= g.num_vertices(); i++) {
            //System.out.println("v = " + i + " pai = " + pai[i] + " dist = " + dist[i]);
            if (pai[i] != -1) {
                max += dist[i];
